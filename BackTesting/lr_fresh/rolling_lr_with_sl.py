...
for i in range(look_back, len(X) - pred_window, pred_window):
    ...
    df.iloc[df[i:i+pred_window].datetime, 'predicted_return'] = model.predict(X[i:i+pred_window])
...
#             tot -= 1
#             df.at[index, 'signal'] = -1
# df.to_csv(f"../src/logs/rolling_lr_{look_back}_{pred_window}_{target_k}.csv")
